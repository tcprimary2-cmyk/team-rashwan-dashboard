...
marker='/* REPORT STYLE COMPATIBILITY FIX V4 */'
if marker in s:
    raise SystemExit('already fixed')
anchor='  /* MONTHLY ALL-AGENT REPORT V1 */'
if anchor not in s:
    raise SystemExit('anchor missing')
css=r'''  /* REPORT STYLE COMPATIBILITY FIX V4 */
  ...
  .rep-kpi-grid{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin:14px 0 16px}
  ...
  .rep-two-col{display:grid!important;grid-template-columns:minmax(0,1.35fr) minmax(0,.9fr);gap:14px;margin:14px 0}
  .rep-card{border:1px solid #e4e7ec;border-radius:12px;padding:14px;background:#fff;min-width:0;overflow:hidden}
  .rep-card h3{font-size:12px;margin:0 0 10px;color:#101828;font-weight:700}
  ...
  .rep-metric-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  ...
  @media(max-width:700px){.rep-kpi-grid,.rep-monthly-agent .rep-kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.rep-two-col{grid-template-columns:1fr}.rep-card{overflow-x:auto}}
  @media print{.rep-kpi-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.rep-monthly-agent .rep-kpi-grid{grid-template-columns:repeat(5,minmax(0,1fr))}.rep-two-col,.rep-card,.rep-kpi-grid,.rep-metric-grid{break-inside:avoid;page-break-inside:avoid}}

'''
s=s.replace(anchor,css+anchor,1)
p.write_text(s,encoding='utf-8')
print('patched report styles')
